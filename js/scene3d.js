/* ==========================================================================
   BEEKUM GARAGE — scene3d.js · "the glass chapel"
   A real-time gothic glass showroom. One Rocket Bunny S13 on a rotating
   dais, a grand piano you can actually play, two guitars that strum, and
   a night city breathing behind seven arched glass bays.
...
const THEMES = {
  /* "day" = EMBER — blood-red neon, candlelight, warm city haze            */
  day: {
    accent: 0xff4d66, accentDim: 0x8f1626,
    fog: 0x0a0509, fogD: 0.017,
    hemiSky: 0x2a1420, hemiGround: 0x060309, hemiI: 0.5,
    candle: 0xffb36b, candleI: 5.0,
    keyLight: 0xffe2d0, keyI: 160,
    cove: 0xff3050, sign: 0xff4d66,
    city: 'warm', moon: 0xffd9b0,
    glowTint: 0xff2b4d
  },
  /* "night" = VIOLET — cold amethyst, silver moon, blue city               */
  night: {
    accent: 0xb48cff, accentDim: 0x4b2a8f,
    fog: 0x06060f, fogD: 0.019,
    hemiSky: 0x191430, hemiGround: 0x040308, hemiI: 0.55,
    candle: 0xffc48a, candleI: 3.4,
    keyLight: 0xdfe4ff, keyI: 150,
    cove: 0x8a5cff, sign: 0xb48cff,
    city: 'cool', moon: 0xdfe8ff,
    glowTint: 0x8a5cff
  }
};
...
export function createGarage(canvas, opts = {}) {
  ...
  function cityTexture(warm) {
    const W = 2048, H = 512;
    const [c, x] = cnv(W, H);
    const sky = x.createLinearGradient(0, 0, 0, H);
    if (warm) {
      sky.addColorStop(0, '#050208'); sky.addColorStop(0.62, '#170812');
      sky.addColorStop(1, '#33101a');
    } else {
      sky.addColorStop(0, '#020208'); sky.addColorStop(0.62, '#0a0a1e');
      sky.addColorStop(1, '#141433');
    }
    x.fillStyle = sky; x.fillRect(0, 0, W, H);
    /* towers */
    let bx = 0;
    while (bx < W) {
      const bw = 34 + Math.random() * 90;
      const bh = H * (0.28 + Math.random() * 0.5);
      x.fillStyle = warm ? 'rgba(16,6,10,0.94)' : 'rgba(6,7,16,0.94)';
      x.fillRect(bx, H - bh, bw, bh);
      /* windows */
      const cols = Math.max(2, bw / 11 | 0), rows = Math.max(3, bh / 13 | 0);
      for (let i = 0; i < cols; i++) for (let j = 0; j < rows; j++) {
        if (Math.random() > 0.24) continue;
        const warmWin = Math.random() < (warm ? 0.8 : 0.35);
        x.fillStyle = warmWin
          ? `rgba(255,${150 + Math.random() * 60 | 0},${70 + Math.random() * 40 | 0},${0.5 + Math.random() * 0.5})`
          : `rgba(${120 + Math.random() * 60 | 0},${170 + Math.random() * 50 | 0},255,${0.4 + Math.random() * 0.5})`;
        x.fillRect(bx + 3 + i * 11, H - bh + 4 + j * 13, 4, 6);
      }
      bx += bw + (Math.random() * 26 | 0);
    }
    /* street haze */
    const haze = x.createLinearGradient(0, H * 0.6, 0, H);
    haze.addColorStop(0, 'rgba(0,0,0,0)');
    haze.addColorStop(1, warm ? 'rgba(255,80,90,0.16)' : 'rgba(120,110,255,0.15)');
  ...
  function archFrame(width, height, archH) {
    /* pointed gothic arch outline as a flat frame                            */
    const w2 = width / 2, springY = height - archH;
    const out = new THREE.Shape();
    out.moveTo(-w2, 0); out.lineTo(-w2, springY);
    out.quadraticCurveTo(-w2, height, 0, height + archH * 0.28);
    out.quadraticCurveTo(w2, height, w2, springY);
    out.lineTo(w2, 0); out.lineTo(-w2, 0);
    const t = 0.11;
    const inn = new THREE.Path();
    const iw = w2 - t, ih = height - t, isp = springY - t * 0.4;
    inn.moveTo(-iw, t); inn.lineTo(-iw, isp);
    inn.quadraticCurveTo(-iw, ih, 0, ih + archH * 0.28 - t * 1.4);
    inn.quadraticCurveTo(iw, ih, iw, isp);
    inn.lineTo(iw, t); inn.lineTo(-iw, t);
    out.holes.push(inn);
    const g = new THREE.ExtrudeGeometry(out, { depth: 0.09, bevelEnabled: false });
    return g;
  }
  ...
  /* rose window — iron tracery circle over the central bay                  */
  const rose = new THREE.Group();
  {
    const R = 1.15;
    rose.add(new THREE.Mesh(new THREE.TorusGeometry(R, 0.055, 10, 48), IRON));
    rose.add(new THREE.Mesh(new THREE.TorusGeometry(R * 0.42, 0.04, 8, 32), IRON));
    for (let i = 0; i < 8; i++) {
      const sp = new THREE.Mesh(new THREE.CylinderGeometry(0.028, 0.028, R * 0.94, 6), IRON);
      sp.position.set(Math.cos(i / 8 * TAU) * R * 0.5 + 0, Math.sin(i / 8 * TAU) * R * 0.5, 0);
      sp.rotation.z = i / 8 * TAU + Math.PI / 2;
      rose.add(sp);
    }
    const rim = new THREE.Mesh(
      new THREE.TorusGeometry(R + 0.09, 0.018, 8, 48),
      mat.std({ color: 0x0c0c10, emissive: 0xb48cff, emissiveIntensity: 1.6, roughness: 0.5, metalness: 0.4 })
    );
  ...
  const moonHalo = new THREE.Mesh(
    ...
    mat.flat({
      map: underglowTexture(), color: 0xb8c4ff, fog: false,
  ...
  const flames = [];       // { mesh, light, seed }
  function chandelier(x, y, z, r = 0.85, candles = 10) {
    const g = new THREE.Group();
    const ring = new THREE.Mesh(new THREE.TorusGeometry(r, 0.045, 10, 40), IRON);
    ring.rotation.x = Math.PI / 2;
    g.add(ring);
    const hub = new THREE.Mesh(new THREE.SphereGeometry(0.09, 12, 10), IRON);
    hub.position.y = 0.55; g.add(hub);
    for (let i = 0; i < 3; i++) {
      const ch = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, ROOM.h - y - 0.05, 5), IRON_SOFT);
      const a = i / 3 * TAU;
      ch.position.set(Math.cos(a) * r * 0.7, (ROOM.h - y) / 2 + 0.02, Math.sin(a) * r * 0.7);
      ch.rotation.x = Math.atan2(Math.sin(a) * r * 0.3, ROOM.h - y) * -Math.sin(a);
      g.add(ch);
      const arm = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, r * 0.72, 5), IRON_SOFT);
      arm.position.set(Math.cos(a) * r * 0.35, 0.28, Math.sin(a) * r * 0.35);
      arm.rotation.z = Math.PI / 2 - a * 0; arm.lookAt(hub.position);
      g.add(arm);
    }
    const candleGeo = new THREE.CylinderGeometry(0.024, 0.028, 0.17, 8);
    const candleMat = mat.std({ color: 0xe8dcc8, roughness: 0.7 });
    const flameGeo = new THREE.ConeGeometry(0.02, 0.06, 8);
    for (let i = 0; i < candles; i++) {
      const a = i / candles * TAU;
      const cx = Math.cos(a) * r, cz = Math.sin(a) * r;
      const cup = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.03, 0.035, 10), IRON);
      cup.position.set(cx, 0.045, cz); g.add(cup);
      const cd = new THREE.Mesh(candleGeo, candleMat);
      cd.position.set(cx, 0.15, cz); g.add(cd);
      const fl = new THREE.Mesh(flameGeo, mat.std({
        color: 0x1a0d02, emissive: 0xffb36b, emissiveIntensity: 8, roughness: 1
      }));
      fl.position.set(cx, 0.27, cz);
      g.add(fl);
      flames.push({ mesh: fl, seed: Math.random() * 100, baseY: 0.27 });
    }
    const light = new THREE.PointLight(0xffb36b, 4.2, 11, 1.9);
    light.position.y = 0.32;
    g.add(light);
    flames.push({ light, seed: Math.random() * 100 });
  ...
  function fallbackCar() {
    /* if the model can't load, park a dark silhouette so the dais isn't bare */
    const s = new THREE.Shape();
    s.moveTo(-2.1, 0.12); s.lineTo(-2.15, 0.5); s.quadraticCurveTo(-2.0, 0.72, -1.3, 0.78);
    s.quadraticCurveTo(-0.5, 1.18, 0.45, 1.14); s.quadraticCurveTo(1.3, 1.05, 1.75, 0.7);
    s.lineTo(2.2, 0.62); s.quadraticCurveTo(2.28, 0.4, 2.2, 0.14); s.lineTo(-2.1, 0.12);
    const body = new THREE.Mesh(
      new THREE.ExtrudeGeometry(s, { depth: 1.7, bevelEnabled: true, bevelThickness: 0.06, bevelSize: 0.06, bevelSegments: 3 }),
      BLACK_GLOSS
    );
    body.rotation.y = Math.PI / 2; body.position.set(0.85, 0.06, -2.1);
    const g = new THREE.Group(); g.add(body);
    const wg = new THREE.CylinderGeometry(0.31, 0.31, 0.24, 24);
    [[-1.32, 0.85], [1.38, 0.85], [-1.32, -0.85], [1.38, -0.85]].forEach(([wz, wx]) => {
      const w = new THREE.Mesh(wg, IRON); w.rotation.z = Math.PI / 2;
      w.position.set(wx, 0.31, wz); g.add(w);
    });
    g.position.y = 0.171;
    carRig.add(g);
    carModel = g;
  ...
  rug.rotation.x = -Math.PI / 2; rug.position.y = 0.006;
  ...
  rugTrim.rotation.x = Math.PI / 2; rugTrim.position.y = 0.012;
  lounge.add(rugTrim);
  ...
  /* trophy vitrine on the left wall                                         */
  {
    ...
    g.add(caseFrame);
    /* arch niche behind */
    const nicheGeo = archFrame(1.7, 2.5, 0.55);
    const niche = new THREE.Mesh(nicheGeo, IRON);
    niche.position.set(0, 0.3, -0.34);
    g.add(niche);
    /* three cups */
    function trophy(h) {
  ...
  function setTheme(t) {
    ...
    rose.userData.rim.material.emissive.setHex(T.accent);
  ...
  function render(dt, now) {
    ...
    if (!reduced) {
      for (let i = 0; i < flames.length; i++) {
        const f = flames[i];
        const n = Math.sin(t * 11 + f.seed) * 0.5 + Math.sin(t * 23.7 + f.seed * 2) * 0.5;
        if (f.mesh) {
          f.mesh.scale.y = 1 + n * 0.22;
          f.mesh.position.y = f.baseY + n * 0.004;
        }
        if (f.light) f.light.intensity = THEMES[themeName].candleI * (1 + n * 0.12);
      }
    }
