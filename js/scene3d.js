...
export function createGarage(canvas, opts = {}) {
  ...
  function cityTexture(warm) {
    ...
    while (bx < W) {
      ...
      for (let i = 0; i < cols; i++) for (let j = 0; j < rows; j++) {
        ...
        const al = 0.24 + Math.random() * 0.4;
        const col = warmWin
          ? [255, 150 + Math.random() * 60 | 0, 70 + Math.random() * 40 | 0]
          : [120 + Math.random() * 60 | 0, 170 + Math.random() * 50 | 0, 255];
        const wx = bx + 3 + i * 11, wy = H - bh + 4 + j * 13;
        /* soft halo first, then a small core — reads as distant glass,
           and stays under the bloom threshold so it can't strobe          */
        x.fillStyle = `rgba(${col[0]},${col[1]},${col[2]},${al * 0.3})`;
        x.fillRect(wx - 1, wy - 1, 5, 7);
        x.fillStyle = `rgba(${col[0]},${col[1]},${col[2]},${al})`;
        x.fillRect(wx, wy, 3, 5);
      }
  ...
  console.info('BEEKUM GARAGE · build 3 — studio showroom');
